...
class EndDevice(XBeeDevice):
    ...
    def __init__(self):
        super().__init__(device_id="XBEE_X", wdt_timeout=60000, battery_pin='D1', battery_scaling_factor=2.9, remote_camera=True, pin_camera='D12')
        ...
        self.deep_sleep = True  # Habilitar deep sleep
                ...
                if self.device_state == self.STATE_STARTUP:
                    ...
                    if self.pin_sensor_general == 1:
                        ...
                        if self.camera_remote:
                            if time.ticks_diff(current_time, self.last_sensor_notification_time) >= self.DEBOUNCE_TIME_MS:  
                                ...
                                print("Esperando para reenviar notificación de sensor: {} segundos restantes".format((self.DEBOUNCE_TIME_MS - time.ticks_diff(current_time, self.last_sensor_notification_time)) // 1000))
                        ...
                        self.turn_off_camera()  # Desactivar cámara si el tiempo ha pasado
